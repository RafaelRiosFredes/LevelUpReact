...
export const DetalleCompra = () => {
  const [carrito, setCarrito] = useState<ProductoCarritoCompleto[]>([]);
  ...
  useEffect(() => {
    ...
    const totalCalc = carritoCompleto.reduce(
            ...
            {carrito.length > 0 ? (
              carrito.map((item, index) => (
                <tr key={index}>
                  <td style={{ width: "80px" }}>
                    <img
                      src={item.imagenes && item.imagenes.length > 0 ? item.imagenes[0] : 'https://via.placeholder.com/60'}
                      alt={item.nombre}
